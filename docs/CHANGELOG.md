...
- **TUI provider form** — Add starts typing the alias immediately; Enter commits each field and continues to the next. Tab moves between fields; `j`/`k` move inside Models. A visible `+ add model` row adds another model (also `+` / `a`); `d` / `-` ask for confirmation before removing. Edit still starts on Base URL in navigation mode so `s` is save.
