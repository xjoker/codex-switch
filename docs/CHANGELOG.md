...
- **TUI provider form** — Add starts typing the alias immediately; Enter commits each field and continues to the next. After a model id is committed the form stays on Models so `+/-`, reasoning, search, default, and `s` work. Edit still starts on Base URL in navigation mode so `s` is save.
- **TUI palette** — Every TUI style now sets the designed dark background with the foreground, so a light terminal (xfce4-terminal / macOS Terminal defaults) cannot wash the yellow keys and cyan headings back to black-on-white.
