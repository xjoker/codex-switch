...
`UNKNOWN`：Windows 本地全量检查通过，仍缺少新提交对应的 macOS/Linux/Windows CI 结果。provider identity 与跨运行 resume 已按确认设计落地。当前没有发布正式版本、移动 dev tag 或合并 master。

## 本地检查

### Windows TUI 输入验证

- 使用隔离的 `CODEX_SWITCH_HOME` 与 `CODEX_HOME`，在 Windows ConPTY 中运行编译后的 TUI，并确认鼠标捕获和退出时终端恢复。
- 实际验证 Tab 点击、账号行双击、弹窗外点击关闭、启动选择器阻止鼠标点击与滚轮穿透、`u` 切换状态和 `o` 启动入口。隔离账号使用测试凭据，没有切换真实账号或修改用户配置。
- 补充真实渲染命中区域测试，覆盖四个 Tab、Provider 行选择和 Modal 隔离；补充滚轮及键盘输入取消待定双击状态的回归测试。

| 检查 | 真实结果 |
|---|---|
| 凭据、启动、TUI、网络与缓存审查 | R1/R2 初审各两项问题，修复后的定向复审通过 |
| 安装、更新与供应链安全审查 | R3 九项问题完成修复并通过定向复审 |
| Windows 测试 | 当前候选 `cargo test --all-targets` 共 721 项通过（574 库 + 147 集成）；早期消融与失败修复详见消融记录 |
| Linux 测试 | 全量 572 库 + 146 集成，共 718 项通过；最后 HTTP fixture 修改后的启动 9 项再验证通过 |
| Clippy | 当前候选 Windows `--all-targets -- -D warnings` 通过；此前 Linux 同入口通过，待新提交 CI 复核 |
...
- provider 的稳定 identity、隔离 run home、provider-scoped ID/name/`--last`/picker resume 已通过 Windows 本地全量测试；仍需新提交的三平台 CI 确认。
