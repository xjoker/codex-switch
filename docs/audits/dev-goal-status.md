...
## 预算计数
- Agent调用：38/50（本goal从0计，启动前审计不计）。
- Review：5/10。
...
- 700a5d3保存R1凭据修复；R1定向复审开始（第4次Review）。A31/TUI修复完成待绿灯。


- R1复审通过，两项关闭；TUI两项红灯转绿，R2定向复审中。
- 安装idle/IEX两项红灯实测exit101；A36实施中。.gitattributes与shell LF已落地，尚待Linux复测。


- R2定向复审通过，两项关闭。A36安装回归Windows 4/4、Linux 2/2通过；A29发布流程已冻结，A38文档/版本进行中。
