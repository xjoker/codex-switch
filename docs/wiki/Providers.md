...
Provider launches create a persistent, isolated Codex home for each invocation under `$CODEX_SWITCH_HOME/provider-runs/<identity_id>/<run_id>/`. The provider has a stable `identity_id` independent of its alias, so concurrent launches of the same or different providers never share Codex sqlite/config state. `launch <provider> resume <SESSION_ID>`, a unique session name, `resume --last`, and bare `resume` (provider-scoped picker) resolve sessions only within that provider identity and pass the exact session ID to Codex. `--last` follows Codex's `cwd`, `--all`, and `--include-non-interactive` semantics; subagent and `exec` sessions are excluded by default. Resume uses the original model unless an explicit `--model` is supplied; if that model is no longer saved, resume fails clearly. A concurrent resume of the same run is rejected immediately. Provider history survives rename; removing a provider deletes its key/profile but leaves a tombstone and history, while recreating the alias receives a new identity. Legacy alias-based runs are migrated under the provider lock on first load.
