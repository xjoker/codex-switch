...
The stored key is never rendered in the table. `o` launches Codex on both tabs: Accounts and Providers open a picker for a model, optional extra Codex argv (Tab), and a one-shot reasoning override, then Enter (or `o`) starts Codex. Accounts lists **(Codex default)** plus slugs already fetched for that profile; confirming stages that ChatGPT `auth.json`. On the Providers list, Enter also opens that picker; `e` edits (including env key, wire API, and extra `-c` overrides). `←`/`→` in the picker change reasoning for this session only (the saved profile is unchanged). `l` is re-login on Accounts, never launch. Codex runs in the foreground; the TUI resumes when it exits.
