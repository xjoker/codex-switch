...
`dev` 是下一正式版发布前的滚动测试通道，可能继续变化。直装用户可运行 `codex-switch self-update --dev`；macOS/Linux 新装使用安装脚本的 `--dev` 参数，Windows PowerShell 设置 `$env:CS_DEV="1"` 后运行安装脚本。安装后用 `codex-switch --version` 确认版本以 `-dev` 结尾（例如 `20260827.2.0-dev`），并用 `codex-switch self-update --check --dev` 检查通道。测试结束后运行 `codex-switch self-update --stable` 回到直装正式版。
