...
- **新增**：打开后直接输入 Alias；`Enter` 提交当前字段并进入下一项。
- **编辑**：从 Base URL 的导航态开始（避免 `s` 被当成输入字符）；`Enter` 进入当前格编辑。
- `Tab` 在 Alias、Base URL、API Key、Models 四栏之间切换；在 Models 内用 `j` / `k` 移动。
...
启动选择器（Providers 上 `Enter` / `o`）：`←` / `→` 只改**本次会话**的 reasoning，不写回配置文件。
...
当前滚动开发版示例：`codex-switch self-update --dev`，版本号形如 `20260826.2.0-dev`。
