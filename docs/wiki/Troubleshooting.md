...
| Some models 404 on `{base_url}/responses` | Codex always POSTs `/responses`. The gateway listed the id (or it was typed) but has no Responses channel for it. Drop it from the saved list (`provider fetch-models` or TUI `f` / `d`) and keep a slug that already works. `/models` listed ≠ usable. |
