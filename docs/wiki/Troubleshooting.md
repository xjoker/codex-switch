...
| Some models 404 on `{base_url}/responses` | Codex POSTs `/responses`. If the model does not support thinking, leave reasoning on `(skip)` so the catalog lists `none` and Codex does not send `reasoning.effort` (example: Cursor `composer-2.5`, whose `/models` row has a `fast` parameter, not effort). If it still 404s, the gateway has no Responses channel for that id — drop it. |
