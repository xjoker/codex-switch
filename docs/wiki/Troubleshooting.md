...
| Custom provider fails with `Server tool request failed` (HTTP 400) | Codex's built-in `web_search` server tool is on by default and most third-party models reject it. Set `web_search = "disabled"` in `$CODEX_HOME/config.toml`, or launch with `-c web_search=disabled`. See [Disable Codex's built-in web_search](Providers#disable-codexs-built-in-web_search-for-third-party-models). |
