...
| `/mcp` empty (or custom prompts missing) after provider `launch` | Older builds used an empty isolated `CODEX_HOME`. Current builds keep the user's `$CODEX_HOME`; only model and endpoint change. Confirm `~/.codex/config.toml` still has `[mcp_servers]` and update with `self-update --dev`. |
