...
| Custom provider fails with `Server tool request failed` (HTTP 400) | The model rejects Codex's built-in `web_search` server tool (some models accept it, some do not). Save `web_search=disabled` on the provider (`--no-web-search` or `--set web_search=disabled`), or pass `-c web_search=disabled` after `--`. Do not edit `$CODEX_HOME/config.toml` for this. See [Model-specific request settings](Providers#model-specific-request-settings). |
| Custom provider warns `Model metadata for '<model>' not found` | `codex-switch launch` writes `$CODEX_SWITCH_HOME/providers/<alias>/models.json` whose slugs cover the provider's `--model` (and `--models`, or the gateway's small `/models` list). Metadata comes from the gateway, then the public OpenRouter catalog. If you set `--set model_catalog_json=…` yourself, that file must include every slug you use. See [Model metadata](Providers#model-metadata). |
