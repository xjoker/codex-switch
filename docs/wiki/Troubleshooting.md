...
| `/mcp` empty (or custom prompts missing) after provider `launch` | Provider launch uses an isolated `CODEX_HOME`. Older builds left that tree empty. Current builds copy MCP, `AGENTS.md`, `prompts/`, and `skills/` from the user's `$CODEX_HOME` at each launch; `auth.json` stays put. The originals in `~/.codex` were not deleted. |
