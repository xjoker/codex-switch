...
| Custom provider warns `Model metadata for '<model>' not found` | `codex-switch launch` writes `$CODEX_SWITCH_HOME/providers/<alias>/models.json` whose `slug` matches the provider's model. If you set `--set model_catalog_json=…` yourself, that file must include the same slug. See [Model metadata](Providers#model-metadata). |
